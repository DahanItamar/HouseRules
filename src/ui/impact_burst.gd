...
func _configure(color: Color, strong: bool) -> void:
	lifetime = 0.62 if strong else 0.42
...
func _configure_gpu(color: Color, strong: bool) -> void:
	...
	gpu.amount = 22 if strong else 12
	...
	process.initial_velocity_min = 70.0 if strong else 42.0
	process.initial_velocity_max = 150.0 if strong else 92.0
	process.gravity = Vector3(0.0, 115.0, 0.0)
...
func _configure_cpu(color: Color, strong: bool) -> void:
	...
	cpu.amount = 22 if strong else 12
	cpu.lifetime = lifetime
	cpu.direction = Vector2.UP
	cpu.spread = 180.0
	cpu.initial_velocity_min = 70.0 if strong else 42.0
	cpu.initial_velocity_max = 150.0 if strong else 92.0
	cpu.gravity = Vector2(0.0, 115.0)
