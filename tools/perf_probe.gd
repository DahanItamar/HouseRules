...
## Godot_v4.7.2-stable_win64_console.exe --path . res://tools/perf_probe.tscn -- --capture-size=1920x1080
...
func _run() -> void:
	...
	for id: String in ["slot_classic", "blackjack", "minefield_vault"]:
...
func _report() -> void:
	...
	print(
		...
		% [
			_phase, sorted.size(), avg, p95, sorted.back() if not sorted.is_empty() else 0.0,
