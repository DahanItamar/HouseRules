...
func test_gamepad_b_dismisses_join_until_player_reenters_ring() -> void:
	var floor := FloorController.new()
	add_child_autofree(floor)
	...
	assert_null(SceneRouter.session, "A cannot trigger a hidden join action")
	assert_eq(floor._dismissed_game, &"slot_classic")
	...
	floor._unhandled_input(accept)
	assert_not_null(SceneRouter.session, "A joins after the player re-enters the ring")
